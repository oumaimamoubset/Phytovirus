...
read_a_given_sheet<-function(filename, sheet_name){
  ...
  readxl::read_excel(
    ...
    pivot_longer(cols =everything(),  
                 names_to = "Quadrat",
                 values_to = "Vegetation",
                 values_drop_na = TRUE) -> Vegetation_by_quadrat
  ...
  readxl::read_excel(
    ...
    mutate(Year = str_sub(grid_name_base, 0,2),
...
ggsave("Distrib_total_cover_by_grid.pdf",width = 20, height = 15, units = "cm")

ggplot(quadrat_data, aes(x=Total_cover, fill=Locality))+
  geom_histogram(position="dodge")+
  ggtitle("Distribution of total cover over all grids of ANG, BDX and CAM")
ggsave("Distrib_total_cover_by_loc.pdf",width = 20, height = 15, units = "cm")
...
ggsave("Vegetation_vs_total_cover.pdf",width = 20, height = 15, units = "cm")
...
ggsave("Richness_diversity.pdf",width = 20, height = 15, units = "cm")
...
ggplot(quadrat_data, aes(x=Plant_richness, y=Plant_diversity))+
  ...
  facet_grid(~Locality)+
  ggtitle("Richness-diversity")
ggsave("Richness_diversity_by_loc.pdf",width = 20, height = 15, units = "cm")
...
ggsave("Distrib_cover_classes.pdf",width = 20, height = 15, units = "cm")

ggplot(plant_data, aes(x=Cover_classes, fill=Locality))+
  geom_histogram(position="dodge")+
  ggtitle("Distribution of cover classes over all grids of ANG, BDX and CAM")
ggsave("Distrib_cover_by_loc.pdf",width = 20, height = 15, units = "cm")

ggplot(plant_data, aes(x=Relative_abundance))+
  geom_histogram()+
  ggtitle("Distribution of relative abundances over all grids of ANG, BDX and CAM")
ggsave("Distrib_rel_ab.pdf",width = 20, height = 15, units = "cm")
...
ggsave("Distrib_rel_abLOG.pdf",width = 20, height = 15, units = "cm")

ggplot(plant_data, aes(x=Relative_abundance, fill=Locality))+
  geom_histogram()+
  ggtitle("Distribution of relative abundances over all grids of ANG, BDX and CAM")
ggsave("Distrib_rel_ab_by_loc.pdf",width = 20, height = 15, units = "cm")

##Abundance profiles----
ggplot(plant_data, aes(x=Rank, y=Relative_abundance))+
  geom_point()+
  stat_summary(fun.y=mean, geom="point", shape=20, size=3, color="orange", fill="orange") +
  theme(axis.text.x = element_text(angle=90))+
  facet_grid(~Locality)+
...
ggplot(plant_data, aes(x=Rank, y=Relative_abundance))+
  geom_point()+
  stat_summary(fun=median, geom="line", size = 1, color="orange") +
  theme(axis.text.x = element_text(angle=90))+
  ggtitle("Abundance profiles")
ggsave("Abundance_profile.pdf",width = 20, height = 15, units = "cm")

ggplot(plant_data, aes(x=Rank, y=Relative_abundance, color=Plant_richness))+
  geom_point()+
  theme(axis.text.x = element_text(angle=90))+
  facet_wrap(~Plant_richness)+
  ggtitle("Abundance profiles in each richness class")
ggsave("Abundance_profile_by_richness.pdf",width = 20, height = 15, units = "cm")

ggplot(plant_data, aes(x=Rank, y=Relative_abundance, col = Locality))+
  geom_point()+
  stat_summary(fun=median, geom="line", size=1, color="black") +
  theme(axis.text.x = element_text(angle=90))+
  facet_grid(~Locality)+
  ggtitle("Abundance profiles")
ggsave("Abundance_profile_by_loc.pdf",width = 20, height = 15, units = "cm")
...
plant_data %>% 
  ...
  theme(axis.text.x = element_text(angle=90))+
  ggtitle("Abundance profiles")
ggsave("Abundance_profile_CAM.pdf",width = 20, height = 15, units = "cm")
