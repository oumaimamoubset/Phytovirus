...
Total plant cover in quadrats may be measured in different ways. A Vegetation index is pifometrically measured in situ. It should more or less relate to Total_cover, the sum of the mean of the cover ranges over all plants, and Total_cover_max, the sum of the superior limit of cover ranges over plants. Total_cover and Total_cover_ max should not be less than Vegetation. Values of Total_cover and Total_cover_ max exceeding Vegetation could indicate that plants are disposed into several layers.
...
NB : Richness as a number of plant species. Diversity as a Hill number of order 1 (i.e., exp(Shannon)).
...
ggplot(quadrat_data, aes(x = Plant_richness, y = Plant_diversity)) +
  ...
  ggtitle("Richness-diversity")
```
NB : It could be good to check the outlier at > 25 species. Maybe Hugo took too much coffee that day...

## Distributions of abundances

Cover ranges can be viewed as measures of absolute abundance. Here the variable Cover_classes accounts for the means of cover ranges.
...
```
Following a bold inspiration, let's compute relative abundances using the normalized Cover_classes (Cover_classes/sum(Cover_classes) for each plant in a quadrat):
...
```
Now we take the log10 of this relative abundance. If the distribution is flat, relative abundance follows a 1/x function (Barbier, pers. comm.): 
...
```
Matthieu says that this is flat. Philippe would probably see a cosinus. Other opinions?
...
Ultimately, Matthieu needs something that may help simulating realistic plant communities, i.e., a distribution of abundance distributions. Here we provide such distributions by omitting plant identities, ranking plants by decreasing relative abundance in each quadrat and plotting the distribution of the abundance of the most frequent plant, of the second most frequent plant and so on... we call that abundance profiles.
...
```
Awfully hard to read... but worthy. Here we separate abundance profiles by richness classes (big boss order). Strangely, it seems that communities with many plants are more democratic than communities with few plants (not dominated by one or two abundant plants). Artefact or not? 
